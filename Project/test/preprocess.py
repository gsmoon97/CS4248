...
if __name__ == "__main__":
    ...
    for track_no in ("1", "3", "0"):
        ...
        maybe_do(eval(f"fp.GUTENBERG_ORI{track_no}"), perturb.do,
                 (word2ptbs, fp.BPE_MODEL, fp.GUTENBERG_TXT,
                  eval(f"fp.GUTENBERG_ORI{track_no}"), eval(
                      f"fp.GUTENBERG_COR{track_no}"), args.n_epochs[0],
        ...
        maybe_do(eval(f"fp.TATOEBA_ORI{track_no}"), perturb.do,
                 (word2ptbs, fp.BPE_MODEL, fp.TATOEBA_TXT,
                  eval(f"fp.TATOEBA_ORI{track_no}"), eval(
                      f"fp.TATOEBA_COR{track_no}"), args.n_epochs[1],
                  args.word_change_prob, args.type_change_prob))

        logging.info("STEP 4-4. wiki103")
        maybe_do(eval(f"fp.WIKI103_ORI{track_no}"), perturb.do,
                 (word2ptbs, fp.BPE_MODEL, fp.WIKI103_TXT,
                  eval(f"fp.WIKI103_ORI{track_no}"), eval(
                      f"fp.WIKI103_COR{track_no}"), args.n_epochs[2],
                  args.word_change_prob, args.type_change_prob))
    ...
    maybe_do(fp.WI_TRAIN_SP_ORI, spell.check,
             (fp.WI_TRAIN_ORI, fp.WI_TRAIN_SP_ORI))
    ...
    maybe_do(fp.WI_TEST_SP_ORI, spell.check,
             (fp.WI_TEST_ORI, fp.WI_TEST_SP_ORI))

    logging.info("STEP 6-7. wi dev 3k")
    maybe_do(fp.WI_DEV_3K_SP_ORI, spell.check,
             (fp.WI_DEV_3K_ORI, fp.WI_DEV_3K_SP_ORI))

    logging.info("STEP 6-8. wi dev 1k")
    maybe_do(fp.WI_DEV_1K_SP_ORI, spell.check,
             (fp.WI_DEV_1K_ORI, fp.WI_DEV_1K_SP_ORI))

    logging.info("STEP 6-9. conll 2013")
    maybe_do(fp.CONLL2013_SP_ORI, spell.check,
             (fp.CONLL2013_ORI, fp.CONLL2013_SP_ORI))

    logging.info("STEP 6-10. conll 2014")
    maybe_do(fp.CONLL2014_SP_ORI, spell.check,
             (fp.CONLL2014_ORI, fp.CONLL2014_SP_ORI))

    #
    logging.info("STEP 7. bpe-tokenize")
    logging.info("STEP 7-1. fce")
    maybe_do(fp.FCE_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.FCE_SP_ORI, fp.FCE_TOK_ORI))
    maybe_do(fp.FCE_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.FCE_COR, fp.FCE_TOK_COR))

    logging.info("STEP 7-2. lang8")
    maybe_do(fp.LANG8_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.LANG8_SP_ORI, fp.LANG8_TOK_ORI))
    maybe_do(fp.LANG8_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.LANG8_COR, fp.LANG8_TOK_COR))

    logging.info("STEP 7-3. nucle")
    maybe_do(fp.NUCLE_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.NUCLE_SP_ORI, fp.NUCLE_TOK_ORI))
    maybe_do(fp.NUCLE_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.NUCLE_COR, fp.NUCLE_TOK_COR))

    logging.info("STEP 7-4. wi train")
    maybe_do(fp.WI_TRAIN_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_TRAIN_SP_ORI, fp.WI_TRAIN_TOK_ORI))
    maybe_do(fp.WI_TRAIN_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_TRAIN_COR, fp.WI_TRAIN_TOK_COR))

    logging.info("STEP 7-5. wi dev")
    maybe_do(fp.WI_DEV_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_DEV_SP_ORI, fp.WI_DEV_TOK_ORI))
    maybe_do(fp.WI_DEV_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_DEV_COR, fp.WI_DEV_TOK_COR))

    logging.info("STEP 7-6. wi test")
    maybe_do(fp.WI_TEST_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_TEST_SP_ORI, fp.WI_TEST_TOK_ORI))
    ...
    maybe_do(fp.WI_DEV_3K_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_DEV_3K_SP_ORI, fp.WI_DEV_3K_TOK_ORI))
    maybe_do(fp.WI_DEV_3K_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_DEV_3K_COR, fp.WI_DEV_3K_TOK_COR))

    logging.info("STEP 7-8. wi dev 1k")
    maybe_do(fp.WI_DEV_1K_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_DEV_1K_SP_ORI, fp.WI_DEV_1K_TOK_ORI))
    maybe_do(fp.WI_DEV_1K_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.WI_DEV_1K_COR, fp.WI_DEV_1K_TOK_COR))

    logging.info("STEP 7-9. conll2013")
    maybe_do(fp.CONLL2013_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.CONLL2013_SP_ORI, fp.CONLL2013_TOK_ORI))
    maybe_do(fp.CONLL2013_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.CONLL2013_COR, fp.CONLL2013_TOK_COR))

    logging.info("STEP 7-10. conll2014")
    maybe_do(fp.CONLL2014_TOK_ORI, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.CONLL2014_SP_ORI, fp.CONLL2014_TOK_ORI))
    maybe_do(fp.CONLL2014_TOK_COR, bpe.bpe_tokenize,
             (fp.BPE_MODEL, fp.CONLL2014_COR, fp.CONLL2014_TOK_COR))
